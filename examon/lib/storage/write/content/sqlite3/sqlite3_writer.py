...
class Sqlite3Writer(ContentWriter):
    def __init__(
        self, engine=None, filename_strategy=None, models: list[BaseQuestion] = None
    ) -> None:
        self.engine = engine
        self.models = models
    ...
    def create_all(self) -> None:
        ...
        with Session(self.engine) as session:
            for model in self.models:
                version_number = 1
                repository = (
                    model.repository if model.repository is not None else "default"
                )
                question_db_record = Question(
                ...
                for index, print_log in enumerate(model.print_logs):
                    question_db_record.print_logs.append(
                        PrintLog(value=print_log, log_number=index)
                    )
