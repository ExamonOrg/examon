...
class TestWriter:
    ...
    def test_creates_question_with_metrics(self):
        with Session(
            Helpers.setup_everything2(FixturesLoader.load_q3_with_choices)
        ) as session:
    ...
    def test_creates_question_with_prints_logs_record(self):
        with Session(
            Helpers.setup_everything2(FixturesLoader.load_q3_with_prints)
        ) as session:
            stmt = select(Question).where(Question.internal_id == "question_three")
            for question in session.scalars(stmt, 0):
                assert list(
                    map(lambda x: (x.value, x.log_number), question.print_logs)
                ) == [("a", 0), ("b", 1), ("c", 2), ("2", 3)]
    ...
    def test_creates_question_with_choices_record(self):
        with Session(
            ...
            stmt = select(Question).where(Question.internal_id == "question_three")
            for question in session.scalars(stmt, 0):
                assert list(map(lambda x: x.value, question.choices)) == ["1", "2", "3"]
